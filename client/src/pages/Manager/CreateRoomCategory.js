...
const CreateRoomCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("/api/v1/roomcategory/create-room-category", {
        name,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    try {
      const { data } = await axios.put(
        `/api/v1/roomcategory/update-room-category/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `/api/v1/roomcategory/delete-room-category/${pId}`
      );
      if (data.success) {
    ...
    <div className="container mt-4">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="md:col-span-3">
          <h1 className="text-2xl font-semibold mb-4">Manage Room Category</h1>
